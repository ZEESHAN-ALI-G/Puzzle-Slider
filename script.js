...
class Game {
  ...
  resetPuzzle() {
    // Reset puzzle to its initial unsolved state
    this.randomize(this.difficulty);
    this.turnCount = 0; // Reset turn count
    document.getElementById("turn_count").textContent = this.turnCount; // Update turn count display
  }
  ...
  resetTurnCount() {
    this.turnCount = 0; // Reset turn count to 0
    document.getElementById("turn_count").textContent = this.turnCount; // Update turn count display
  }
  ...
  onClickOnBlock(blockIdx) {
    ...
    if (this.moveBlock(blockIdx)) {
      // Increment turn count when a block is moved
      this.turnCount++;

      if (this.checkPuzzleSolved()) {
        setTimeout(() => {
          alert("Puzzle Solved in " + this.turnCount + " turns!!");
          this.resetTurnCount(); // Reset turn count when puzzle is solved
        }, 600);
      }

      // Update turn count display
      document.getElementById("turn_count").textContent = this.turnCount;
    }
  }
...
difficulty_buttons.forEach((elem, idx) => {
  elem.addEventListener("click", (e) => {
    ...
    game.resetTurnCount(); // Reset turn count when difficulty is changed
  });
});

document.getElementById("reshuffle_button").addEventListener("click", () => {
  game.resetPuzzle(); // Reset and reshuffle the puzzle
});
